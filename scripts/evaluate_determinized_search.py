...
_CONTEXTUAL_BATCHES = tuple(
    ...
    for index, seed_start in enumerate(range(3502, 3802, 50))
)
_CONTEXTUAL_SEARCH = {
    "samples": 6,
    "horizon_antes": 1,
...
def _validate_contextual_preregistration(
    ...
    *,
    ...
    retired_ranges = (range(1075, 2275), range(2602, 3502))
    reserved = range(3502, 3802)
    requested = range(args.seed_start, args.seed_start + args.seeds)
    if any(
        ...
        raise SystemExit(
            "contextual v9-v16 seeds 1075-2274 and 2602-3501 are retired"
        )
    overlaps_reserved = (
        requested.start < reserved.stop and reserved.start < requested.stop
    )
    ...
    if path is None:
        if overlaps_reserved:
            raise SystemExit(
                "seeds 3502-3801 require --contextual-preregistration-json"
            )
        return None
    try:
        raw = path.read_bytes()
    ...
    if (
        spec.get("protocol_id") != "contextual-continuation-development-v9"
        or spec.get("status") != "reserved"
        or spec.get("immutable_batches") is not True
    ):
