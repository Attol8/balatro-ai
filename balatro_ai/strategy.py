...
def retrieve_examples(observation: PublicObservation) -> list[dict[str, str]]:
    ...
    owned = {item.key for item in (*observation.jokers, *observation.consumables)
             if isinstance(item, PublicItem) and not item.debuffed}
    owned.update(observation.used_vouchers)
    offered = {item.key for item in (*observation.shop, *observation.opened_pack, *observation.vouchers)
    ...
    for blind in observation.blinds:
        if (blind.kind == "BOSS" and not blind.disabled
    ...
    for example in load_examples():
        ...
        if (observation.phase.value not in example["phases"]
                or not triggers.intersection(facts) or not required.issubset(facts)):
            continue
        # Prefer decisions about an actual offer, then more specific contexts.
        priority = (len(triggers.intersection(offered)), len(required),
    ...
    for _, example in matches:
        ...
        result.append({key: example[key] for key in
                       ("id", "situation", "lesson", "reversal")})
