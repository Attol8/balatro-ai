...
- Commit `f8b4d9a` added the optional Python 3.11 recurrent model: deterministic public feature hashing, a single GRU cell, dynamic legal-action scoring, a value head, strict checkpoint loading, and an explicit no-reorder enumerative proposal surface. It passes hidden-twin, order-invariance, candidate-mask, reset, serialization, and finite-gradient tests. It is randomly initialized and has not won a game. The first live rollout proved that a 512-candidate bound is too small once a build raises hand size; the declared fail-closed model bound is now 2,048, enough to enumerate play/discard choices through 11 cards. Larger hands require a factorized tactical head rather than silent candidate truncation.
...
- Start with clipped PPO over bounded `no_reorder_enumerated_max2048_v1` candidates, fixed-length contiguous recurrent rollouts, GAE, normalized advantages, gradient clipping, and exact recurrent resets. Truncations bootstrap from their final public observation but never carry advantage into the next episode. Do not backpropagate through private environment state or reconstruct actions from simulator IDs.
- Keep `sparse_terminal_v1` as the promotion objective. A separately named `public_progress_v1` training reward may add only a bounded public round-progress delta; reports must separate environment reward from shaped training reward, and final evaluation uses sparse terminal outcomes only.
