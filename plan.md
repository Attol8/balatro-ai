...
- The next seed-3 divergence exposed an undeclared environment input: the local career profile had Mr. Bones unlocked and Seeing Double locked while Jackdaw assumes a fully unlocked profile. Benchmark authority launches now use a transient `all_unlocked` mode equivalent to Balatro's normal profile action, report it through health, and seal it in the trace manifest. Scripts fail closed on a missing or different profile mode; career-profile runs are diagnostics only. Under that contract, `runs/differential-dev/mixed-v3-all-unlocked-seed3/red-white-seed3.jsonl` exercises both choosing and skipping packs, passes trace-derived mixed-lane coverage, and replays exactly for all 21/21 public transitions. It also regressed Troubadour's hand-reset timing: the reduced hand count becomes visible after blind selection, not on the preceding blind screen. Regenerate the skip and pick lanes under schema v2 before extending coverage.
